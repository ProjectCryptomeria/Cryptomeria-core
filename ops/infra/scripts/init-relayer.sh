...
if [ -z "$CHAIN_NAMES_CSV" ]; then
  echo "Error: CHAIN_NAMES_CSV must be set."
  exit 1
fi
...
if [ ! -f "$RELAYER_HOME/config/config.yaml" ]; then
    ...
    for CHAIN_ID in $CHAIN_IDS; do
        # Docker Composeのサービス名はChain IDと一致させている前提
        RPC_ADDR="http://${CHAIN_ID}:26657"
        GRPC_ADDR="http://${CHAIN_ID}:9090"
...
EOF
    ...
    for CHAIN_ID in $CHAIN_IDS; do
        ...
        # ファイルが存在するまで少し待つ（マウントタイミング等のため）
        MAX_WAIT=30
        WAIT_COUNT=0
        while [ ! -f "$MNEMONIC_FILE" ]; do 
            if [ $WAIT_COUNT -ge $MAX_WAIT ]; then echo "Error: Mnemonic $MNEMONIC_FILE not found."; exit 1; fi
            echo "Waiting for mnemonic file $MNEMONIC_FILE..."
            sleep 1
    ...
    for CHAIN_ID in $CHAIN_IDS; do
        ...
        until [ $(curl -s "${RPC_ADDR}/status" | jq -r '.result.sync_info.latest_block_height // "0"' || echo "0") -ge 5 ]; do
            echo "Waiting for $CHAIN_ID to produce blocks..."
            sleep 1
        done
