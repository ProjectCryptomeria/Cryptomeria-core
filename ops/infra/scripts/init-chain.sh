...
if [ ! -d "$CHAIN_HOME/config" ]; then
    ...
    if [ -f "$MNEMONIC_FILE" ]; then
        ...
        echo "WARNING: Mnemonic file not found at $MNEMONIC_FILE. Generating new keys..."
        # 開発用に新規生成する場合のロジックが必要ならここに追加
    fi

    # アドレス取得 (失敗時は空文字になるためエラーハンドリング推奨だが、今回は簡略化)
    VALIDATOR_ADDR=$($CHAIN_BINARY keys show validator -a --keyring-backend=test --home "$CHAIN_HOME" 2>/dev/null || true)
    RELAYER_ADDR=$($CHAIN_BINARY keys show relayer -a --keyring-backend=test --home "$CHAIN_HOME" 2>/dev/null || true)
    CREATOR_ADDR=$($CHAIN_BINARY keys show creator -a --keyring-backend=test --home "$CHAIN_HOME" 2>/dev/null || true)
    ...
    if [ "$CHAIN_APP_NAME" = "gwc" ]; then
        ...
        # 環境変数からエンドポイントを取得、未定義の場合はDockerサービス名をデフォルトとする
        MDSC_ENDPOINT=${MDSC_ENDPOINT:-"http://mdsc:1317"}
        FDSC_0_ENDPOINT=${FDSC_0_ENDPOINT:-"http://fdsc-0:1317"}

        cat <<EOF >> "$APP_TOML"

[gwc]
mdsc_endpoint = "$MDSC_ENDPOINT"
chunk_size = 10240
[gwc.fdsc_endpoints]
fdsc = "$FDSC_0_ENDPOINT"
fdsc-0 = "$FDSC_0_ENDPOINT"
# fdsc-1 = ... (If needed, add logic or env var)
EOF
    fi
