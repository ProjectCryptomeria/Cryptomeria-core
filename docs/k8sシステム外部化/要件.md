...
* **[決定事項 1-1] システムアカウントの仕様**
    * **種類:** 各ノードは`local-admin`,`relayer`アカウントを持つ。
        * `local-admin`:バリデーター兼富豪アカウントとして振る舞う
        * `relayer`:IBC用のアカウント
    * **鍵の管理:** 鍵（ニーモニック）はValues.yamlで静的に管理する。FDSC-Nは全て同一のニーモニックを使用する。
    * **初期残高:** 
        * `relayer`:最初の接続を行うのに十分かつ過剰すぎない額として `100000uatom` を設定。
        * `local-admin`:`1000000000000uatom`を設定。
