...
- 方式A：APIサーバーをクラスタ内にデプロイ（推奨）
  - `ServiceAccount` + `RBAC` で必要権限を付与
  - in-cluster config を使用（kubeconfig不要）
- 方式B：APIサーバーをローカル/別VMで実行
  - `KUBECONFIG`（またはtoken/endpoint）
  - クラスタへ到達可能なネットワーク

必要権限（最小セット目安）
- get/list/watch: pods, services, endpoints, statefulsets, deployments
- create/patch: jobs（任意）, statefulsets（スケール時）
