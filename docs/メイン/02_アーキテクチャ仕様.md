...
- **Atomic Upload**: 全ての fragment 保存（ACK）を確認した後でのみ、manifest を MDSC に送信する（整合性保証）。
- manifest を MDSC に送信
- endpoint registry（MDSC/FDSC の API URL）を保持
- **Service Discovery**: Download/Hosting 時に、オンチェーンの `StorageInfo` から動的に接続先を解決し、復元を行う（ハードコード排除）。
