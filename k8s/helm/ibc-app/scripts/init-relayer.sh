...
EOF
    ...
    for DATA_CHAIN_ID in $DATA_CHAIN_IDS; do
        PATH_NAME="${PATH_PREFIX}-${META_CHAIN_ID}-to-${DATA_CHAIN_ID}"
        ...
        rly transact channel "$PATH_NAME" --src-port metastore --dst-port datastore --order unordered --version "ibc-proto-1" -t 30s -r 5 --log-level warn
