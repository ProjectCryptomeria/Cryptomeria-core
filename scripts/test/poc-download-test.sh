...
# Pod名とIP/Service名の取得
# Kubernetesクラスタ内部（GWC Pod内）からアクセスするためのURLを構築
# GWC Podの中から他のPodへは、KubernetesのDNS名でアクセス可能
...
log "   Backend MDSC: $MDSC_URL"
log "   Backend FDSC: $FDSC_URL"

# GWC Podの中で gwcd コマンドを実行
# 出力先をPod内の /tmp に指定し、あとで取り出して検証する
