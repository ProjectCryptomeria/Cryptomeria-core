...
const CONFIG = {
    ...
    MEMPOOL_TX_LIMIT: 50, // Mempoolがこの数未満になるまで待機
...
class ChainManager {
    ...
    public async broadcastSequentialTxs(
        ...
        // 💡 修正: ノンス情報は外部から受け取る
        // AccountNumberは変わらないと仮定し、接続エラー時でも再取得しない
        // (もしAccountNumberも変わる可能性がある場合は、ensureConnectedなどで再取得が必要)
        ...
        bar.update(completedTxOffset + completedTxCountInBatch, { status: 'Waiting for inclusion...' }); 
        ...
        const results = await Promise.all(inclusionPromises.map((p, index) => p.then(result => {
            ...
            bar.update(totalCompleted, { 
                ...
                tx_per_sec: txPerSec, 
                status: `Confirming (${completedTxCountInBatch}/${totalTxsInBatch})` 
            });
            return result;
        }).catch(e => {
            throw e;
    ...
    // 💡 削除: waitForBatchInclusion (broadcastSequentialTxs に統合されたため)
    ...
    while (true) {
        try {
            ...
            try {
                ...
                bar.update(currentValue, { status: `Reconnected. Retrying...` });
...
/**
 * 💡 修正点: パイプライン処理を廃止し、Mempoolチェック + 同期バッチ送信に戻す
 */
async function executeDistributionWorkers(chainManager: ChainManager, megaJobsByChain: Map<string, MegaChunkJob[]>, dataChains: ChainInfo[], estimatedGas: number): Promise<void> {
    ...
    const workerPromises = dataChains.map(chain => {
        ...
        return (async () => {
            ...
            // 💡 修正点: パイプラインを廃止し、同期バッチループ + Mempoolチェックに戻す
            try {
                for (let batchIndex = 0; batchIndex < messageBatches.length; batchIndex++) {
                    const batchMessages = messageBatches[batchIndex]!;
                    const batchStartTime = Date.now();
                    
                    // 💡 (1) Mempoolチェック ＆ ヘルスチェック
                    bar.update(completedTxCountInWorker, { status: `Batch ${batchIndex + 1}/${messageBatches.length} Checking mempool...` });
                    await waitForMempoolSpace(chainManager, chainName, bar, completedTxCountInWorker);
                    
                    // 💡 (1b) もしノンスが初期化されていなければ、再接続後に再取得
                    if (batchIndex === 0 && currentSequenceRef.sequence === 0 && accountInfo?.sequence === 0) {
                    ...
                    const results = await chainManager.broadcastSequentialTxs(
                        chainName, 
                        batchMessages, 
                        estimatedGas, 
                        bar,
                        completedTxCountInWorker,
                        currentSequenceRef // 💡 ノンス参照を渡す
                    );
                    
                    completedTxCountInWorker += results.length;
                    const batchTimeSec = ((Date.now() - batchStartTime) / 1000).toFixed(2);
                    
                    bar.update(completedTxCountInWorker, { 
                        status: `Batch ${batchIndex + 1}/${messageBatches.length} Done (${batchTimeSec}s)`,
                        height: results[results.length - 1]?.height 
                    });
                }
                ...
                // 次のループ（または終了）の前に、Mempoolチェックが再接続を試みる
...
async function main() {
    ...
    try {
        ...
        logger.info('[MAIN] Starting distributed sequential chunk uploads (Noncing + HealthCheck + Mempool aware)...');
